"""Phase 4 tier test runner.

Winner combo from Phase 3 v2: qwen3.5-9b + qwen3.5-27b.
Reviewer: mistral (cross-family, low variance).
Tiers: strong, mid, weak (1 cascade each, 3 total).

Records go to iterative_benchmark_results.jsonl under phase="tier_v2".
"""
import json
import os
import sys
...
# Load all three cached stage sets
cache_sets = {}
for tier in TIERS:
    ...
    with open(RESULTS_FILE) as f:
        for line in f:
            try:
                ...
                continue
    if done_keys:
        print(f"Resuming: {len(done_keys)} tier runs already complete")

combo_key = f"{GEN}_{RW}_{REV}"
total = len(TIERS)

print(f"\nPhase 4 (tier_v2): {total} cascades")
print(f"Combo: {GEN} + {RW} + {REV}")
print(f"Tiers: {', '.join(TIERS)}\n")

STATUS_FILE.write_text(f"Phase 4 started: {datetime.now().isoformat()}\n{total} tier cascades\n")
...
for idx, tier in enumerate(TIERS, 1):
    ...
    try:
        ...
        print(f"  FAILED: {e}")
        notify(f"Phase 4 {idx}/{total}: tier={tier} FAILED — {e}")
        record = {
            "phase": PHASE,
            "combo_key": combo_key,
            "generator": GEN,
    ...
    STATUS_FILE.write_text(
        f"Phase 4: {idx}/{total} done\n"
        f"Last: tier={tier} = {grade} ({passed}/{task_count})\n"
        f"Time: {datetime.now().isoformat()}\n"
    )

    notify(f"Phase 4 [{idx}/{total}]: tier={tier} = {grade} ({passed}/{task_count})")

notify("Phase 4 COMPLETE")
print("\nPhase 4 COMPLETE")
