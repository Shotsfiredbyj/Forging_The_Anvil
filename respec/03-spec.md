...
**The first artefact: a vision the user can feel.** Annie produces a short, confident statement of what she thinks the user is trying to build, who it's for, and what "done" looks like for a first version. This is rendered in the conversation, not delivered as a markdown download. The user reads it and reacts. Annie takes the reaction and edits the vision in place. This loop is tight — seconds, not minutes. *(The vision also persists as `docs/vision.md` inside the project — a dual-purpose artefact that is both a conversational beat and a document the user owns. See `07-creative-memory.md` for the full creative-artefact framing.)*
